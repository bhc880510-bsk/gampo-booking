...
# 골프존 감포CC의 클럽 시퀀스 코드
GAMPO_GOLFCLUB_SEQ = "1"

# [수정] 앱 제목 변경
st.set_page_config(
    page_title="골프존 감포CC 예약",
    page_icon="⛳",
...
# ============================================================
# Session State Initialization (누락된 초기화 코드 추가)
# ============================================================
...
# ============================================================
# API Booking Core Class (골프존 감포CC 전용)
# ============================================================
class APIBookingCore:
    def __init__(self, log_func, message_queue, stop_event):
        ...
        self.GAMPO_SEQ = GAMPO_GOLFCLUB_SEQ  # "1"
    ...
    def keep_session_alive(self, target_dt):
        ...
        # 감포CC 예약 페이지 (가장 기본적인 GET 요청)
        keep_alive_url = f"{self.API_DOMAIN}/reserve/main/teetimeList?golfclubSeq={self.GAMPO_SEQ}"
...
#                "selectPersonCnt": "4",
    ...
    def filter_and_sort_times(self, all_times_html, start_time_str, end_time_str, target_course_names, is_reverse):
        ...
        try:
            ...
            for li in available_list_items:
                try:
                    ...
                    course_nm = course_span.text if course_span else "알수없음"
        ...
        # UI에서 선택한 코스 이름을 기준으로 필터링
        if target_course_names == "OUT":
            allowed_course_names = ["OUT"]
        elif target_course_names == "IN":
            allowed_course_names = ["IN"]
        elif target_course_names == "ALL":
            allowed_course_names = ["IN", "OUT"]  # 감포CC의 일반적인 코스 이름 가정

        for time_info in parsed_times:
            # time_info[3] is course_nm (e.g., 'IN' or 'OUT')
            if time_info[3] in allowed_course_names:
    ...
    def try_reservation(self, date, time_table_id, course_cd_code, time_api, course_name):
        ...
        referer_url_step1 = f"{self.API_DOMAIN}/reserve/main/teetimeList?golfclubSeq={self.GAMPO_SEQ}"
...
def start_pre_process(message_queue, stop_event, inputs):
    ...
    try:
        ...
        # 골프존 카운티는 로그인 후 바로 예약 페이지 GET 요청으로 세션을 안정화할 수 있습니다.
        try:
            core.session.get(f"{core.API_DOMAIN}/reserve/main/teetimeList?golfclubSeq={core.GAMPO_SEQ}", timeout=5.0,
                             verify=False)
